...
Kyma CLI has been used for cluster management. We should look into how we can introduce support for management of components in the Kyma cluster. This would bring in following benefits:
...
* resource folder consists of the yaml of the function cr that has been deployed in the kyma cluster. This would be generated before function is being deployed. It would be generated on every deployment.
...
Out of the box from the init command we should give an easy example which can be extended by the developer later. The example should be possible to be deployed directly in the kyma cluster. Here below there is a node example:
...
* Once the function has been deployed and if the function has been modified in the cluster using commands listed at [expanded commands with switches](#expanded-command-with-switches), then the local `config.yaml` is out of sync. Re applying via `kyma function apply` can result in confusion.
