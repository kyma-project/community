...
If you use a third-party CRD, apply the [location and file name](./03-custom-resource-definitions.md#location-and-file-name) recommendations from this guide. Keep the content unaltered if the technical requirements allow it. The content must comply with the company's software usage policy and the third-party CRD's license.
...
- **spec:version**: Each API Group can exist in multiple versions. Use the version name in the URL, for example `v1alpha1`, `v1beta1`, or `v1`. For more details, see the [Consideration](./03-custom-resource-definitions.md#consideration) section. For more information on versioning CRDs, see the [Versioning](#custom-resource-definition-custom-resource-definition-versioning) section.
