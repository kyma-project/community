...
A central collector, which listens for OTLP metrics, will be part of the setup. The related service port can be used to push custom metrics of any kind. That collector will be auto-scaled depending on resource consumption. Also, that instance will be configurable via the telemetry operator's MetricPipeline resource.
