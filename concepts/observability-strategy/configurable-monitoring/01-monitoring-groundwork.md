...
### General
- Users need a way to outstream metrics into multiple external systems. There will be no production-ready backend provided by Kyma, so user need to integrate with something.
...
- Have a vendor-neutral layer of agents/gateways that collects and ships metrics, but does not permanently store it (as a backend).
