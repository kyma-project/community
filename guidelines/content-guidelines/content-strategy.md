...
There is a set of documents that a given technical topic must include. You can also add additional document types to expose a specific topic better. To propose a new document type, create an [issue](https://github.com/kyma-project/kyma/issues) in the `kyma` repository and mark it with the `area/documentation` label.  
